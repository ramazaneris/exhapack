...
let createProject = (answers, targetDir, projectName) => {
	let packageJson = {
		...
		scripts: {
			start: answers.typescript
				? `ts-node ${answers.sourceDir ? answers.sourceDir + "/" : ""}index.ts`
				: `node ${answers.sourceDir ? answers.sourceDir + "/" : ""}index.js`,
			dev: answers.typescript
				? `ts-node-dev --respawn ${answers.sourceDir ? answers.sourceDir + "/" : ""}index.ts`
				: `nodemon ${answers.sourceDir ? answers.sourceDir + "/" : ""}index.js`,
		},
		license: "MIT",
		keywords: [],
	};
